...
// Alle remote Branches abrufen - Priorisiere github-branches.json
function getRemoteBranches() {
    // ZUERST: Prüfe ob github-branches.json existiert (echte GitHub-Daten)
    if (fs.existsSync(GITHUB_BRANCHES_FILE)) {
        try {
            const githubBranches = JSON.parse(fs.readFileSync(GITHUB_BRANCHES_FILE, 'utf8'));
            return githubBranches;
        } catch (e) {
            console.error('Fehler beim Lesen der GitHub-Branches:', e.message);
        }
    }
    
    // FALLBACK: Verwende git branch -r (wahrscheinlich veraltet!)
    console.log(`${colors.yellow}  ⚠️  Keine github-branches.json gefunden, verwende Git-Cache (kann veraltet sein!)${colors.reset}`);
...
function fetchRemoteStatus() {
    ...
    // Prüfe ob github-branches.json existiert und aktuell ist
    if (fs.existsSync(GITHUB_BRANCHES_FILE)) {
        const stats = fs.statSync(GITHUB_BRANCHES_FILE);
        const ageInHours = (Date.now() - stats.mtime) / (1000 * 60 * 60);
        
        if (ageInHours < 24) {
            console.log(`${colors.green}✓ Verwende GitHub-Daten aus github-branches.json${colors.reset}`);
            return;
        } else {
            console.log(`${colors.yellow}⚠️  github-branches.json ist ${Math.floor(ageInHours)} Stunden alt - bitte aktualisieren!${colors.reset}`);
        }
    } else {
        console.log(`${colors.yellow}⚠️  Keine github-branches.json gefunden - erstelle sie mit den echten GitHub-Branches!${colors.reset}`);
    }
    
    // Versuche trotzdem git fetch (für lokale Änderungen)
    gitCommand('git fetch --all --quiet 2>/dev/null');
}
