...
function analyzeProjectStorage(projects) {
    ...
    for (const [name, project] of Object.entries(projects)) {
        ...
        if (!project || !project.path) {
            console.log(`  ${icons.error} ${colors.red}Invalid project configuration (no path)${colors.reset}`);
            console.log(`  ${icons.warning} Run: cn -d ${name} to remove this entry`);
            continue;
        }
        ...
        if (!projectCheck.exists) {
            ...
            console.log(`  ${icons.warning} Project may have been moved or deleted`);
            continue;
        }
    ...
    for (const [name, project] of Object.entries(analysis.projects || {})) {
        ...
        if (!project || !project.path) {
            recommendations.push({
                type: 'error',
                message: `Project '${name}' has invalid configuration (missing path)`,
                action: `Remove with: cn -d ${name}`
            });
            continue;
        }
...
async function runDoctor(options = {}) {
    ...
    if (projectCount > 0) {
        ...
        // Check if project is valid before accessing path
        if (firstProject && firstProject.path) {
            ...
            storageMode = 'invalid (corrupt project data)';
        }
    }
